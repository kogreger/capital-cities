...
plotDhk09DistSex <- ggplot(dhk09.person, aes(dist / 1000, fill = sex)) +
  ...
  labs(title = "Dhaka 2009, Distance Traveled", 
       x = "Distance [km]", 
       y = "Density") +
...
# load person data incl. total trip distance
query <- "SELECT r.pid, MIN(r.sex) sex, MIN(r.age) age, MIN(r.occup) occup, MIN(r.occupc) occupc, SUM(t.dist) dist FROM hni04.person r JOIN dhk09.trip t ON r.pid = t.pid GROUP BY r.pid ORDER BY r.pid;"
...
plotHni04DistSex <- ggplot(hni04.person, aes(dist / 1000, fill = sex)) +
  ...
  scale_x_continuous(labels = comma) +
  scale_fill_brewer(palette = "Set1", name = "Sex", drop = FALSE) + 
  labs(title = "Hanoi 2004, Distance Traveled", 
       x = "Distance [km]", 
       y = "Density") +
  theme(legend.justification = c(1, 1), legend.position = c(1, 1), 
        legend.key = element_blank())
...
# load person data incl. total trip distance
query <- "SELECT r.pid, MIN(r.sex) sex, MIN(r.age) age, MIN(r.occup) occup, MIN(r.occupc) occupc, SUM(t.dist) dist FROM jkt02.person r JOIN dhk09.trip t ON r.pid = t.pid GROUP BY r.pid ORDER BY r.pid;"
...
plotJkt02DistSex <- ggplot(jkt02.person, aes(dist / 1000, fill = sex)) +
  ...
  scale_x_continuous(labels = comma) +
  scale_fill_brewer(palette = "Set1", name = "Sex", drop = FALSE) + 
  labs(title = "Jakarta 2001, Distance Traveled", 
       x = "Distance [km]", 
       y = "Density") +
  theme(legend.justification = c(1, 1), legend.position = c(1, 1), 
        legend.key = element_blank())
...
plotMnl96DistSex <- ggplot(mnl96.person, aes(dist / 1000, fill = sex)) +
  ...
  scale_x_continuous(labels = comma) +
  scale_fill_brewer(palette = "Set1", name = "Sex", drop = FALSE) + 
  labs(title = "Metro Manila 1996, Distance Traveled", 
       x = "Distance [km]", 
       y = "Density") +
  theme(legend.justification = c(1, 1), legend.position = c(1, 1), 
        legend.key = element_blank())
...
grid.arrange(plotDhk09DistSex, plotHni04DistSex, 
             plotJkt02DistSex, plotMnl96DistSex, 
             ncol = 2, 
             main = "Distance Traveled, by Sex")
grid.arrange(plotDhk09DistAge, plotHni04DistAge, 
             plotJkt02DistAge, plotMnl96DistAge, 
             ncol = 2, 
             main = "Distance Traveled, by Age")
grid.arrange(plotDhk09DistOccupC, plotHni04DistOccupC, 
             plotJkt02DistOccupC, plotMnl96DistOccupC, 
             ncol = 2, 
             main = "Distance Traveled, by Occupation Group")
```
...
plotDhk09CommTripStartHourAbs <- ggplot(dhk09.commbehav, 
  ...
  labs(title = "Dhaka 2009, Commuting Trips Started per Hour", 
       x = "Time", 
       y = "Count") +
  theme(legend.justification = c(1, 1), legend.position = c(1, 1), 
        legend.key = element_blank())
...
plotHni04CommTripStartHourAbs <- ggplot(hni04.commbehav, 
  ...
  scale_x_discrete(breaks = seq(0, 24, 3)) +
  scale_color_brewer(palette = "Set1", name = "Activity") + 
  labs(title = "Hanoi 2004, Commuting Trips Started per Hour", 
       x = "Time", 
       y = "Count") +
  theme(legend.justification = c(1, 1), legend.position = c(1, 1), 
        legend.key = element_blank())
...
plotJkt02CommTripStartHourAbs <- ggplot(jkt02.commbehav, 
  ...
  scale_x_discrete(breaks = seq(0, 24, 3)) +
  scale_color_brewer(palette = "Set1", name = "Activity") + 
  labs(title = "Jakarta 2002, Commuting Trips Started per Hour", 
       x = "Time", 
       y = "Count") +
  theme(legend.justification = c(1, 1), legend.position = c(1, 1), 
        legend.key = element_blank())
...
# @@@ preprocess data in database first - load dummy data for now
mnl96.commbehav <- dhk09.commbehav[activity == "", ]
# # load commuting behavior data
# query <- "WITH subtrip_info AS (SELECT pid, tno, COUNT(sno) subtrips, MAX(tmodec) tmodec FROM mnl96.subtrip GROUP BY pid, tno) SELECT t.pid, t.tno, t.purpose, t.activity, date_part('hour', t.pdate_s) phour_s, s.subtrips, t.dist, t.pdate_s, t.pdate_e, r.sex, r.age, r.occupc FROM mnl96.trip t JOIN subtrip_info s ON t.pid = s.pid AND t.tno = s.tno JOIN mnl96.person r ON t.pid = r.pid WHERE t.purpose IN (1, 2, 6)  AND (SELECT activity FROM mnl96.trip x WHERE x.pid = t.pid AND x.tno = (t.tno - 1)) IN (1, 2, 3) AND s.tmodec != 97 ORDER BY pid, tno;"
...
plotMnl96CommTripStartHourAbs <- ggplot(mnl96.commbehav, 
  ...
  scale_x_discrete(breaks = seq(0, 24, 3)) +
  scale_color_brewer(palette = "Set1", name = "Activity") + 
  labs(title = "Metro Manila 1996, Commuting Trips Started per Hour", 
       x = "Time", 
       y = "Count") +
  theme(legend.justification = c(1, 1), legend.position = c(1, 1), 
        legend.key = element_blank())
...
```{r plotCommBehavAll, eval = TRUE, echo = FALSE, fig.width = 10, fig.height = 5, cache = TRUE}
grid.arrange(plotDhk09CommBehavAgeNoon, plotHni04CommBehavAgeNoon, 
             plotJkt02CommBehavAgeNoon, plotMnl96CommBehavAgeNoon, 
             ncol = 2, 
             main = "Average Commuting Distance by Age")
grid.arrange(plotDhk09CommBehavOccupCNoon, plotHni04CommBehavOccupCNoon, 
             plotJkt02CommBehavOccupCNoon, plotMnl96CommBehavOccupCNoon, 
             ncol = 2, 
...
grid.arrange(plotDhk09CommTripStartHourAbs, plotHni04CommTripStartHourAbs, 
             plotJkt02CommTripStartHourAbs, plotMnl96CommTripStartHourAbs, 
             ncol = 2, 
             main = "Commuting Trips Started per Hour")
grid.arrange(plotDhk09CommTripStartHourRel, plotHni04CommTripStartHourRel, 
             plotJkt02CommTripStartHourRel, plotMnl96CommTripStartHourRel, 
             ncol = 2, 
             main = "Commuting Trips Started per Hour")
```
...
plotDhk09STDist <- ggplot(dhk09.stdist, aes(x = phour, y = pcount)) + 
  ...
  scale_y_continuous(labels = comma) + 
  scale_x_discrete(breaks = seq(0, 24, 3)) +
  scale_color_discrete(h = gradientDistance, name = "Buffer", drop = FALSE) + 
