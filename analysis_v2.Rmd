...
linestyleDistance <- rep(c("solid", "longdash", "twodash", "dashed", "dotted"), 2)
```
...
dhk09.person <- psqlGetTable(con, "dhk092", "person")
# factorize pid
...
# load person data incl. total trip distance
hni04.person <- psqlGetTable(con, "hni042", "person")
# factorize pid
...
# load person data incl. total trip distance
jkt02.person <- psqlGetTable(con, "jkt022", "person")
# factorize pid
...
# load person data incl. total trip distance
mnl96.person <- psqlGetTable(con, "mnl962", "person")
# factorize pid
...
# load trip data
query <- "WITH non_stationary AS (SELECT pid, tno FROM dhk092.subtrip WHERE tmodec != 97 GROUP BY pid, tno) SELECT t.* FROM dhk092.trip t JOIN non_stationary s ON t.pid = s.pid AND t.tno = s.tno ORDER BY t.pid, t.tno;"
...
# load trip data
query <- "WITH non_stationary AS (SELECT pid, tno FROM hni042.subtrip WHERE tmodec != 97 GROUP BY pid, tno) SELECT t.* FROM hni042.trip t JOIN non_stationary s ON t.pid = s.pid AND t.tno = s.tno ORDER BY t.pid, t.tno;"
...
# load trip data
query <- "WITH non_stationary AS (SELECT pid, tno FROM jkt022.subtrip WHERE tmodec != 97 GROUP BY pid, tno) SELECT t.* FROM jkt022.trip t JOIN non_stationary s ON t.pid = s.pid AND t.tno = s.tno ORDER BY t.pid, t.tno;"
...
# load trip data
query <- "WITH non_stationary AS (SELECT pid, tno FROM mnl962.subtrip WHERE tmodec != 97 GROUP BY pid, tno) SELECT t.* FROM mnl962.trip t JOIN non_stationary s ON t.pid = s.pid AND t.tno = s.tno ORDER BY t.pid, t.tno;"
...
# load subtrip data
dhk09.subtrip <- psqlGetTable(con, "dhk092", "subtrip")
# remove unnecessary column (purpose, puid_s, puid_e)
...
# load subtrip data
hni04.subtrip <- psqlGetTable(con, "hni042", "subtrip")
# remove unnecessary column (purpose, puid_s, puid_e)
...
# load subtrip data
jkt02.subtrip <- psqlGetTable(con, "jkt022", "subtrip")
# remove unnecessary column (purpose, puid_s, puid_e)
...
# load subtrip data
mnl96.subtrip <- psqlGetTable(con, "mnl962", "subtrip")
# remove unnecessary column (purpose, puid_s, puid_e)
...
query <- "WITH subtrip_info AS (SELECT pid, tno, COUNT(sno) subtrips, MAX(tmodec) tmodec FROM dhk092.subtrip GROUP BY pid, tno) SELECT t.pid, t.tno, t.purpose, t.activity, date_part('hour', t.pdate_s) phour_s, s.subtrips, t.dist, t.pdate_s, t.pdate_e, r.sex, r.age, r.occupc FROM dhk092.trip t JOIN subtrip_info s ON t.pid = s.pid AND t.tno = s.tno JOIN dhk092.person r ON t.pid = r.pid WHERE t.purpose IN (1, 2, 3) AND (SELECT activity FROM dhk092.trip x WHERE x.pid = t.pid AND x.tno = (t.tno - 1)) IN (1, 2, 3) AND s.tmodec != 97 ORDER BY pid, tno;"
...
# load commuting behavior data
query <- "WITH subtrip_info AS (SELECT pid, tno, COUNT(sno) subtrips, MAX(tmodec) tmodec FROM hni042.subtrip GROUP BY pid, tno) SELECT t.pid, t.tno, t.purpose, t.activity, date_part('hour', t.pdate_s) phour_s, s.subtrips, t.dist, t.pdate_s, t.pdate_e, r.sex, r.age, r.occupc FROM hni042.trip t JOIN subtrip_info s ON t.pid = s.pid AND t.tno = s.tno JOIN hni042.person r ON t.pid = r.pid WHERE t.purpose IN (1, 2, 3) AND (SELECT activity FROM hni042.trip x WHERE x.pid = t.pid AND x.tno = (t.tno - 1)) IN (1, 2, 3) AND s.tmodec != 97 ORDER BY pid, tno;"
...
# load commuting behavior data
query <- "WITH subtrip_info AS (SELECT pid, tno, COUNT(sno) subtrips, MAX(tmodec) tmodec FROM jkt022.subtrip GROUP BY pid, tno) SELECT t.pid, t.tno, t.purpose, t.activity, date_part('hour', t.pdate_s) phour_s, s.subtrips, t.dist, t.pdate_s, t.pdate_e, r.sex, r.age, r.occupc FROM jkt022.trip t JOIN subtrip_info s ON t.pid = s.pid AND t.tno = s.tno JOIN jkt022.person r ON t.pid = r.pid WHERE t.purpose IN (1, 2, 6) AND (SELECT activity FROM jkt022.trip x WHERE x.pid = t.pid AND x.tno = (t.tno - 1)) IN (1, 2, 3) AND s.tmodec != 97 ORDER BY pid, tno;"
...
```{r loadCommBehavMnl96, eval = TRUE, echo = FALSE, cache = TRUE}
# load commuting behavior data
query <- "WITH subtrip_info AS (SELECT pid, tno, COUNT(sno) subtrips, MAX(tmodec) tmodec FROM mnl962.subtrip GROUP BY pid, tno) SELECT t.pid, t.tno, t.purpose, t.activity, date_part('hour', t.pdate_s) phour_s, s.subtrips, t.dist, t.pdate_s, t.pdate_e, r.sex, r.age, r.occupc FROM mnl962.trip t JOIN subtrip_info s ON t.pid = s.pid AND t.tno = s.tno JOIN mnl962.person r ON t.pid = r.pid WHERE t.purpose IN (1, 2, 3) AND (SELECT activity FROM mnl962.trip x WHERE x.pid = t.pid AND x.tno = (t.tno - 1)) IN (1, 2, 3) AND s.tmodec != 97 ORDER BY pid, tno;"
mnl96.commbehav <- psqlQuery(con, query)
# remove unnecessary column (purpose)
mnl96.commbehav <- mnl96.commbehav[, c("pid", "tno", "activity", "phour_s", 
                                       "subtrips", "dist", "pdate_s", "pdate_e", 
                                       "sex", "age", "occupc")]
# factorize pid
mnl96.commbehav$pid <- factor(mnl96.commbehav$pid)
# factorize tno
mnl96.commbehav$tno <- factor(mnl96.commbehav$tno)
# factorize activity
mnl96.commbehav$activity <- factor(mnl96.commbehav$activity, 
                                   levels = c(1:6, 99), 
                                   labels = labelsActivity[, 2])
# factorize phour_s
mnl96.commbehav$phour_s <- factor(mnl96.commbehav$phour_s, 
                                  levels = c(0:23))
# calculate duration
mnl96.commbehav$duration <- with(mnl96.commbehav, 
                                 pdate_e - pdate_s)
# divide into noon and afternoon
mnl96.commbehav$noon <- factor(cut(as.numeric(mnl96.commbehav$phour_s), 
                                   c(0, 12, 24), 
                                   labels = labelsNoon[, 2]))
# remove NA sex (=0) and factorize
mnl96.commbehav <- mnl96.commbehav[mnl96.commbehav$sex != 0, ]
mnl96.commbehav$sex <- factor(mnl96.commbehav$sex, 
                              levels = c(1, 2), 
                              labels = labelsSex[, 2])
# factorize age (ordered)
mnl96.commbehav$age <- factor(mnl96.commbehav$age, 
                              levels = c(0:17), 
                              ordered = TRUE, 
                              labels = labelsAge[, 2])
# factorize occupation class
mnl96.commbehav$occupc <- factor(mnl96.commbehav$occupc, 
                                 levels = c(1:5, 99), 
                                 labels = labelsOccupC[, 2])
```
...
```{r descCommBehavMnl96, results = "asis", cache = TRUE}
...
```{r plotCommBehavMnl96, eval = TRUE, echo = FALSE, fig.width = 7, fig.height = 7, cache = TRUE}
plotMnl96CommBehavAgeNoon <- ggplot(mnl96CommBehavAgeNoon, 
                                    aes(age, mean_dist / 1000, color = noon)) +
  geom_line(aes(group = noon)) +
  scale_y_continuous(labels = comma) +
  scale_x_discrete(limits = labelsAge[, 2]) + 
  scale_color_discrete(h = gradientAge, drop = FALSE) + 
  labs(title = "Metro Manila 1996, Average Commuting Distance by Age", 
       x = "Age", y = "Average Commuting Distance [km]") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  theme(legend.justification = c(1, 0), legend.position = c(1, 0), 
        legend.key = element_blank(), legend.title = element_blank())
plotMnl96CommBehavOccupCNoon <- ggplot(mnl96CommBehavOccupCNoon, 
                                       aes(occupc, mean_dist / 1000, 
                                           fill = noon)) +
  geom_bar(stat = "identity", position = "dodge", aes(group = noon)) +
  scale_y_continuous(labels = comma) +
  scale_x_discrete(limits = labelsOccupC[, 2]) + 
  scale_fill_brewer(palette = "Set1", drop = FALSE) + 
  labs(title = "Metro Manila 1996, Average Commuting Distance by Occupation Group", 
       x = "Age", y = "Average Commuting Distance [km]") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  theme(legend.justification = c(1, 1), legend.position = c(1, 1), 
        legend.key = element_blank(), legend.title = element_blank())
plotMnl96CommTripStartHourAbs <- ggplot(mnl96.commbehav, 
  ...
  theme(legend.justification = c(1, 1), legend.position = c(1, 1), 
        legend.key = element_blank())
plotMnl96CommTripStartHourRel <- ggplot(mnl96.commbehav, 
                                        aes(phour_s, fill = activity)) +
  geom_bar(position = "fill", width = 1) +
  scale_y_continuous(labels = comma) + 
  scale_x_discrete(breaks = seq(0, 24, 3)) +
  ...
  theme(legend.justification = c(1, 0), legend.position = c(1, 0), 
        legend.key = element_blank())
grid.arrange(plotMnl96CommBehavAgeNoon, plotMnl96CommBehavOccupCNoon, 
             plotMnl96CommTripStartHourAbs, plotMnl96CommTripStartHourRel, 
             ncol = 1)
```
...
query <- "SELECT buff, date_part('hour', pdate) phour, COUNT(DISTINCT pid) pcount FROM dhk092.point p GROUP BY buff, date_part('hour', pdate) ORDER BY date_part('hour', pdate), buff;"
...
# load spatio-temporal distribution data
query <- "WITH non_stationary AS (SELECT pid, tno, sno, MIN(tmodec) FROM dhk092.subtrip WHERE tmodec != 97 GROUP BY pid, tno, sno ORDER BY pid, tno, sno) SELECT p.buff, date_part('hour', p.pdate) phour, COUNT(DISTINCT p.pid) pcount FROM dhk092.point p JOIN non_stationary s ON p.pid = s.pid AND p.tno = s.tno AND p.sno = s.sno GROUP BY p.buff, date_part('hour', p.pdate) ORDER BY date_part('hour', p.pdate), buff;"
...
```{r loadStationarySTDistributionDhk09, eval = TRUE, echo = FALSE, cache = TRUE}
# load spatio-temporal distribution data
query <- "WITH stationary AS (SELECT pid, tno, sno, MIN(tmodec) FROM dhk092.subtrip WHERE tmodec = 97 GROUP BY pid, tno, sno ORDER BY pid, tno, sno) SELECT p.buff, date_part('hour', p.pdate) phour, COUNT(DISTINCT p.pid) pcount FROM dhk092.point p JOIN stationary s ON p.pid = s.pid AND p.tno = s.tno AND p.sno = s.sno GROUP BY p.buff, date_part('hour', p.pdate) ORDER BY date_part('hour', p.pdate), buff;"
dhk09.stdist_stat <- psqlQuery(con, query)
# factorize buff
dhk09.stdist_stat$buff <- factor(dhk09.stdist_stat$buff, 
                                    levels = seq(5, 50, 5), 
                                    labels = paste(seq(5, 50, 5), rep("km", 10)))
# change column name for pretty legend title
names(dhk09.stdist_stat) <- c("Buffer", "phour", "pcount")
```

```{r plotStationarySTDistributionDhk09, eval = TRUE, echo = FALSE, fig.width = 7, fig.height = 7, cache = TRUE}
plotDhk09StatSTDist <- ggplot(dhk09.stdist_stat, aes(x = phour, y = pcount, 
                                                           color = Buffer, 
                                                           linetype = Buffer)) + 
  geom_line() + 
  labs(title = "Spatio-Temporal Distribution of Stationary People, Dhaka", 
       x = "Time", y = "People") + 
  scale_y_continuous(labels = comma) + 
  scale_x_discrete(breaks = seq(0, 24, 3)) +
  scale_color_discrete(h = gradientDistance) + 
  theme(legend.key = element_blank())
grid.arrange(plotDhk09StatSTDist, 
             ncol = 1)
```
...
# load spatio-temporal distribution data
query <- "SELECT buff, date_part('hour', pdate) phour, COUNT(DISTINCT pid) pcount FROM hni042.point p GROUP BY buff, date_part('hour', pdate) ORDER BY date_part('hour', pdate), buff;"
...
# load spatio-temporal distribution data
query <- "WITH non_stationary AS (SELECT pid, tno, sno, MIN(tmodec) FROM hni042.subtrip WHERE tmodec != 97 GROUP BY pid, tno, sno ORDER BY pid, tno, sno) SELECT p.buff, date_part('hour', p.pdate) phour, COUNT(DISTINCT p.pid) pcount FROM hni042.point p JOIN non_stationary s ON p.pid = s.pid AND p.tno = s.tno AND p.sno = s.sno GROUP BY p.buff, date_part('hour', p.pdate) ORDER BY date_part('hour', p.pdate), buff;"
...
```{r loadStationarySTDistributionHni04, eval = TRUE, echo = FALSE, cache = TRUE}
# load spatio-temporal distribution data
query <- "WITH stationary AS (SELECT pid, tno, sno, MIN(tmodec) FROM hni042.subtrip WHERE tmodec = 97 GROUP BY pid, tno, sno ORDER BY pid, tno, sno) SELECT p.buff, date_part('hour', p.pdate) phour, COUNT(DISTINCT p.pid) pcount FROM hni042.point p JOIN stationary s ON p.pid = s.pid AND p.tno = s.tno AND p.sno = s.sno GROUP BY p.buff, date_part('hour', p.pdate) ORDER BY date_part('hour', p.pdate), buff;"
hni04.stdist_stat <- psqlQuery(con, query)
# factorize buff
hni04.stdist_stat$buff <- factor(hni04.stdist_stat$buff, 
                                 levels = seq(5, 50, 5), 
                                 labels = paste(seq(5, 50, 5), rep("km", 10)))
# change column name for pretty legend title
names(hni04.stdist_stat) <- c("Buffer", "phour", "pcount")
```

```{r plotStationarySTDistributionHni04, eval = TRUE, echo = FALSE, fig.width = 7, fig.height = 7, cache = TRUE}
plotHni04StatSTDist <- ggplot(hni04.stdist_stat, aes(x = phour, y = pcount, 
                                                     color = Buffer, 
                                                     linetype = Buffer)) + 
  geom_line() + 
  labs(title = "Spatio-Temporal Distribution of Stationary People, Hanoi", 
       x = "Time", y = "People") + 
  scale_y_continuous(labels = comma) + 
  scale_x_discrete(breaks = seq(0, 24, 3)) +
  scale_color_discrete(h = gradientDistance) + 
  theme(legend.key = element_blank())
grid.arrange(plotHni04StatSTDist, 
             ncol = 1)
```
...
# load spatio-temporal distribution data
query <- "SELECT buff, date_part('hour', pdate) phour, COUNT(DISTINCT pid) pcount FROM jkt022.point p GROUP BY buff, date_part('hour', pdate) ORDER BY date_part('hour', pdate), buff;"
...
# load spatio-temporal distribution data
query <- "WITH non_stationary AS (SELECT pid, tno, sno, MIN(tmodec) FROM jkt022.subtrip WHERE tmodec != 97 GROUP BY pid, tno, sno ORDER BY pid, tno, sno) SELECT p.buff, date_part('hour', p.pdate) phour, COUNT(DISTINCT p.pid) pcount FROM jkt022.point p JOIN non_stationary s ON p.pid = s.pid AND p.tno = s.tno AND p.sno = s.sno GROUP BY p.buff, date_part('hour', p.pdate) ORDER BY date_part('hour', p.pdate), buff;"
...
plotJkt02NonStatSTDist <- ggplot(jkt02.stdist_nonstat, aes(x = phour, y = pcount, 
                                                           ...
                                                           linetype = Buffer)) + 
  geom_line() + 
  labs(title = "Spatio-Temporal Distribution of Moving People, Jakarta", 
       x = "Time", y = "People") + 
  scale_y_continuous(labels = comma) + 
  scale_x_discrete(breaks = seq(0, 24, 3)) +
  scale_color_discrete(h = gradientDistance) + 
...
```{r loadStationarySTDistributionJkt02, eval = TRUE, echo = FALSE, cache = TRUE}
# load spatio-temporal distribution data
query <- "WITH stationary AS (SELECT pid, tno, sno, MIN(tmodec) FROM jkt022.subtrip WHERE tmodec = 97 GROUP BY pid, tno, sno ORDER BY pid, tno, sno) SELECT p.buff, date_part('hour', p.pdate) phour, COUNT(DISTINCT p.pid) pcount FROM jkt022.point p JOIN stationary s ON p.pid = s.pid AND p.tno = s.tno AND p.sno = s.sno GROUP BY p.buff, date_part('hour', p.pdate) ORDER BY date_part('hour', p.pdate), buff;"
jkt02.stdist_stat <- psqlQuery(con, query)
# factorize buff
jkt02.stdist_stat$buff <- factor(jkt02.stdist_stat$buff, 
                                 levels = seq(5, 50, 5), 
                                 labels = paste(seq(5, 50, 5), rep("km", 10)))
# change column name for pretty legend title
names(jkt02.stdist_stat) <- c("Buffer", "phour", "pcount")
```

```{r plotStationarySTDistributionJkt02, eval = TRUE, echo = FALSE, fig.width = 7, fig.height = 7, cache = TRUE}
plotJkt02StatSTDist <- ggplot(jkt02.stdist_stat, aes(x = phour, y = pcount, 
                                                     color = Buffer, 
                                                     linetype = Buffer)) + 
  geom_line() + 
  labs(title = "Spatio-Temporal Distribution of Stationary People, Jakarta", 
       x = "Time", y = "People") + 
  scale_y_continuous(labels = comma) + 
  scale_x_discrete(breaks = seq(0, 24, 3)) +
  scale_color_discrete(h = gradientDistance) + 
  theme(legend.key = element_blank())
grid.arrange(plotJkt02StatSTDist, 
             ncol = 1)
```

### Metro Manila 1996

```{r loadSTDistributionMnl96, eval = TRUE, echo = FALSE, cache = TRUE, cache = TRUE}
# load spatio-temporal distribution data
query <- "SELECT buff, date_part('hour', pdate) phour, COUNT(DISTINCT pid) pcount FROM mnl962.point p GROUP BY buff, date_part('hour', pdate) ORDER BY date_part('hour', pdate), buff;"
mnl96.stdist <- psqlQuery(con, query)
# factorize buff
mnl96.stdist$buff <- factor(mnl96.stdist$buff, 
                           levels = seq(5, 50, 5), 
                           labels = paste(seq(5, 50, 5), rep("km", 10)))
# change column name for pretty legend title
names(mnl96.stdist) <- c("Buffer", "phour", "pcount")
```

```{r plotSTDistributionMnl96, eval = TRUE, echo = FALSE, fig.width = 7, fig.height = 7, cache = TRUE}
plotMnl96STDist <- ggplot(mnl96.stdist, aes(x = phour, y = pcount, 
                                            color = Buffer, linetype = Buffer)) + 
  geom_line() + 
  labs(title = "Spatio-Temporal Distribution of People, Metro Manila", 
       x = "Time", y = "People") + 
  scale_y_continuous(labels = comma) + 
  scale_x_discrete(breaks = seq(0, 24, 3)) +
  scale_color_discrete(h = gradientDistance) + 
  theme(legend.key = element_blank())
grid.arrange(plotMnl96STDist, 
             ncol = 1)
```

```{r loadNonStationarySTDistributionMnl96, eval = TRUE, echo = FALSE, cache = TRUE}
# load spatio-temporal distribution data
query <- "WITH non_stationary AS (SELECT pid, tno, sno, MIN(tmodec) FROM mnl962.subtrip WHERE tmodec != 97 GROUP BY pid, tno, sno ORDER BY pid, tno, sno) SELECT p.buff, date_part('hour', p.pdate) phour, COUNT(DISTINCT p.pid) pcount FROM mnl962.point p JOIN non_stationary s ON p.pid = s.pid AND p.tno = s.tno AND p.sno = s.sno GROUP BY p.buff, date_part('hour', p.pdate) ORDER BY date_part('hour', p.pdate), buff;"
mnl96.stdist_nonstat <- psqlQuery(con, query)
# factorize buff
mnl96.stdist_nonstat$buff <- factor(mnl96.stdist_nonstat$buff, 
                                    levels = seq(5, 50, 5), 
                                    labels = paste(seq(5, 50, 5), rep("km", 10)))
# change column name for pretty legend title
names(mnl96.stdist_nonstat) <- c("Buffer", "phour", "pcount")
```

```{r plotNonStationarySTDistributionMnl96, eval = TRUE, echo = FALSE, fig.width = 7, fig.height = 7, cache = TRUE}
plotMnl96NonStatSTDist <- ggplot(mnl96.stdist_nonstat, aes(x = phour, y = pcount, 
                                                           color = Buffer, 
                                                           linetype = Buffer)) + 
  geom_line() + 
  labs(title = "Spatio-Temporal Distribution of Moving People, Metro Manila", 
       x = "Time", y = "People") + 
  scale_y_continuous(labels = comma) + 
  scale_x_discrete(breaks = seq(0, 24, 3)) +
  scale_color_discrete(h = gradientDistance) + 
  theme(legend.key = element_blank())
grid.arrange(plotMnl96NonStatSTDist, 
             ncol = 1)
```

```{r loadStationarySTDistributionMnl96, eval = TRUE, echo = FALSE, cache = TRUE}
# load spatio-temporal distribution data
query <- "WITH stationary AS (SELECT pid, tno, sno, MIN(tmodec) FROM mnl962.subtrip WHERE tmodec = 97 GROUP BY pid, tno, sno ORDER BY pid, tno, sno) SELECT p.buff, date_part('hour', p.pdate) phour, COUNT(DISTINCT p.pid) pcount FROM mnl962.point p JOIN stationary s ON p.pid = s.pid AND p.tno = s.tno AND p.sno = s.sno GROUP BY p.buff, date_part('hour', p.pdate) ORDER BY date_part('hour', p.pdate), buff;"
mnl96.stdist_stat <- psqlQuery(con, query)
# factorize buff
mnl96.stdist_stat$buff <- factor(mnl96.stdist_stat$buff, 
                                 levels = seq(5, 50, 5), 
                                 labels = paste(seq(5, 50, 5), rep("km", 10)))
# change column name for pretty legend title
names(mnl96.stdist_stat) <- c("Buffer", "phour", "pcount")
```

```{r plotStationarySTDistributionMnl96, eval = TRUE, echo = FALSE, fig.width = 7, fig.height = 7, cache = TRUE}
plotMnl96StatSTDist <- ggplot(mnl96.stdist_stat, aes(x = phour, y = pcount, 
                                                     color = Buffer, 
                                                     linetype = Buffer)) + 
  geom_line() + 
  labs(title = "Spatio-Temporal Distribution of Stationary People, Metro Manila", 
       x = "Time", y = "People") + 
  scale_y_continuous(labels = comma) + 
  scale_x_discrete(breaks = seq(0, 24, 3)) +
  scale_color_discrete(h = gradientDistance) + 
  theme(legend.key = element_blank())
grid.arrange(plotMnl96StatSTDist, 
             ncol = 1)
```
